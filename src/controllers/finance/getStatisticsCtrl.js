...
function countStatistics(list) {
  const getSum = list
    .map(tr => Number(tr.sum))
    .reduce((acc, value) => {
      const sum = acc + value;
      return sum;
    }, 0);
  const total = getSum.toFixed(2);
  return total;
}
...
function getStatisticsByCategory(monthList) {
  ...
  const outlayCategoryList = outlayList
    .map(tr => tr.category)
    .filter((val, ind, arr) => arr.indexOf(val) === ind);
  const filterTrListByCategories = outlayCategoryList.map(cat =>
    outlayList.filter(el => cat === el.category),
  );
